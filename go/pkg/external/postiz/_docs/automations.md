...
This guide mirrors the Playbook format used by the other Ferret API SDKs and
...
| `POSTIZ_RATE_LIMIT` | Optional – overrides default 30 req/hr limit alerts. |
...
jobs:
  publish:
    ...
    steps:
      ...
      - run: go run ./cmd/postiz-publisher
```

`cmd/postiz-publisher` can call the SDK to pull queued Ferret posts, find the
...
- If you proxy requests through Ferret, ensure the `Authorization` header is
