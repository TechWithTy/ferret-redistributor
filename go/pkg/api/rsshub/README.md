...
Typed helper for [RSSHub](https://rsshub.app) so Ferret services can query the official API (`/api/routes`, `/api/radar/search`, `/api/version`, `/api/force-refresh`) and fetch rendered feeds without re-implementing HTTP plumbing.
