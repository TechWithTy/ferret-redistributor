...
This directory hosts the API clients we vendor with Ferret. Some are first-party packages that follow the same scaffolding as `postiz` or `glif`, while others are third-party SDKs pulled in as git submodules so we can track upstream changes directly.
...
- `postiz/` — Ferret’s generated Postiz Public API SDK (v1).
- `recurpost/` — Ferret’s generated RecurPost SDK with tests and Postman collection.
