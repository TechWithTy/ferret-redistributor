...
func authMiddleware() gin.HandlerFunc {
    return func(c *gin.Context) {
        ...
        if sqlDB == nil { c.AbortWithStatusJSON(http.StatusServiceUnavailable, gin.H{"error":"db unavailable"}); return }
        var userID string
        ...
        if !revoked.Time.IsZero() || time.Now().After(expiresAt) { c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error":"session expired or revoked"}); return }
