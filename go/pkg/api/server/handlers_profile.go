...
func getProfile(c *gin.Context) {
    ...
    if err == sql.ErrNoRows {
        c.JSON(http.StatusOK, gin.H{"profile": nil}); return
    } else if err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()}); return }
    if orgID.Valid { p.OrgID = &orgID.String }
    if display.Valid { p.DisplayName = &display.String }
    if avatar.Valid { p.AvatarURL = &avatar.String }
    if bio.Valid { p.Bio = &bio.String }
    if tz.Valid { p.Timezone = &tz.String }
    if loc.Valid { p.Locale = &loc.String }
