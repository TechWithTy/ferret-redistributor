...
func getICP(c *gin.Context) {
    ...
    if uid == "" { c.JSON(http.StatusUnauthorized, gin.H{"error":"unauthorized"}); return }
    // Derive org_id from profile
    var orgID sql.NullString
    _ = sqlDB.QueryRow(`SELECT org_id FROM user_profiles WHERE user_id=$1`, uid).Scan(&orgID)
    if !orgID.Valid { c.JSON(http.StatusOK, gin.H{"icp": nil}); return }
    ...
    if err == sql.ErrNoRows { c.JSON(http.StatusOK, gin.H{"icp": nil}); return }
...
func updateICP(c *gin.Context) {
    ...
    if err := sqlDB.QueryRow(`SELECT COALESCE(org_id,'') FROM user_profiles WHERE user_id=$1`, uid).Scan(&orgID); err != nil || orgID == "" {
        c.JSON(http.StatusBadRequest, gin.H{"error":"no org associated with profile"}); return }
    var req types.ICPUpdate
    if err := c.ShouldBindJSON(&req); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()}); return }
    // Upsert by (org_id, name='Default')
    name := "Default"
    if req.Name != nil && *req.Name != "" { name = *req.Name }
    gb, _ := json.Marshal(req.Goals); pb, _ := json.Marshal(req.Pains); bb, _ := json.Marshal(req.BrandVoice)
    cb, _ := json.Marshal(req.Compliance); ab, _ := json.Marshal(req.Audience); pb2, _ := json.Marshal(req.ContentPillars)
