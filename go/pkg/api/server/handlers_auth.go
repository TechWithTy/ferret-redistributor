...
func signupHandler(c *gin.Context) {
    ...
    if sqlDB == nil { c.JSON(http.StatusServiceUnavailable, gin.H{"error":"db unavailable"}); return }
    // Hash password (requires secure build tag)
    hash, err := auth.HashPassword(req.Password)
    if err != nil { c.JSON(http.StatusNotImplemented, gin.H{"error":"password hashing not enabled"}); return }
...
func loginHandler(c *gin.Context) {
    ...
    if !auth.CheckPassword(hash, req.Password) { c.JSON(http.StatusUnauthorized, gin.H{"error":"invalid credentials"}); return }
    // If JWT configured, issue JWT; else issue opaque session
    if secret := os.Getenv("JWT_SECRET"); strings.TrimSpace(secret) != "" {
        ttl := 24 * time.Hour
        if v := os.Getenv("JWT_TTL"); v != "" { if d, err := time.ParseDuration(v); err == nil { ttl = d } }
    ...
    if _, err := sqlDB.Exec(`INSERT INTO auth_sessions(id, user_id, token_hash, created_at, last_seen_at, expires_at) VALUES($1,$2,$3,NOW(),NOW(),$4)`, sid, uid, th, exp); err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()}); return }
...
func nullOr(s string) any { if s == "" { return nil }; return s }
